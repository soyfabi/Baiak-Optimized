...
mType.onAppear = function(monster, creature)
	...
	for i = 1, 5 do
		local summonName = monster:getType():getSummonList()[math.random(1, #monster:getType():getSummonList())].name
		local summon = Game.createMonster(summonName, monster:getPosition(), true)
		if summon then
			if monster.addSummon then
				monster:addSummon(summon)
			else
				summon:setMaster(monster)
			end
			summon:getPosition():sendMagicEffect(CONST_ME_TELEPORT)
			summon:setStorageValue(Storage.TheOrderOfTheLion.Drume.Commander, 1)
		end
	end
