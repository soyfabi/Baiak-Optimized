local raid = GlobalEvent("Warwolf_raids")
raid:type("timer")
raid:interval(1800)

local function warning()
	Game.broadcastMessage("Many aggressive beasts are roaming south of Ab'Dendriel! Take care!", MESSAGE_STATUS_WARNING)
end

local function spawnWave(from, to, z, monsterName, count)
    for _ = 1, count do
        local x, y = math.random(from.x, to.x), math.random(from.y, to.y)
        local monster = Game.createMonster(monsterName, Position(x, y, z))
        if monster then
            monster:setStorageValue(monsterRaidStorages.FONTICAK_TOKEN, 1)
        end
    end
end

local function warwolf_wave()
    local from, to, z = {x = 32600, y = 31712}, {x = 32634, y = 31739}, 7
    local wolfCount = 55
    local warWolfCount = 70
    spawnWave(from, to, z, "Wolf", wolfCount)
    spawnWave(from, to, z, "War Wolf", warWolfCount)
end

function raid.onTime(interval)
	addEvent(warning, 1000)
    addEvent(warwolf_wave, 1300)

    local currentTime = os.date("%Y-%m-%d %H:%M:%S")
    print(string.format("--> Raid: War Wolf (Ab'dendriel). [Executed: %s]", currentTime))
    return true
end

--raid:register()
