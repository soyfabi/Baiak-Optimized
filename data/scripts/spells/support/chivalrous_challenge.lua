...
local function chain(player)
	...
	while (totalChain < 5 and #monsters > 0) do
		...
		if lastChainPosition:getDistance(closestMonsterPosition) == 1 then
			...
			-- Llamada a getPathTo desde la criatura, no desde Position
			path = lastChain:getPathTo(closestMonsterPosition, 0, 1, true, true, 9)
		...
		if updateLastChain then
			...
			doChallengeCreature(player, closestMonster) -- Esta función debería estar definida para establecer el comportamiento de ataque
