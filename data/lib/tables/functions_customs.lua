...
function Guild.setBankBalance(guildId, amount, playerName, transactionType)
    local query = nil
    if transactionType == "withdraw" then
        query = db.query(string.format(
            "UPDATE `guilds` SET `balance` = `balance` + %d, `namewithdraw` = '%s', `amountwithdraw` = %d WHERE `id` = %d",
            amount, playerName, math.abs(amount), guildId
        ))
    elseif transactionType == "deposit" then
        query = db.query(string.format(
            "UPDATE `guilds` SET `balance` = `balance` + %d, `namedeposit` = '%s', `amountdeposit` = %d WHERE `id` = %d",
            amount, playerName, math.abs(amount), guildId
        ))
    end
    return query ~= false
...
function Guild.getBankBalance(guildId)
    local resultId = db.storeQuery(string.format('SELECT `balance` FROM `guilds` WHERE `id` = %d', guildId))
    if not resultId then
        return 0
    end
...
function Player.addDusts(self, amount)
	return db.query(string.format("UPDATE `players` SET `forge_dusts` = `forge_dusts` + %d WHERE `id` = %d", amount, self:getGuid()))
end

function Player.getDusts(self)
    local resultId = db.storeQuery(string.format('SELECT `forge_dusts` FROM `players` WHERE `id` = %d', self:getGuid()))
    if not resultId then
        return 0
    end

    local value = result.getNumber(resultId, "forge_dusts")
    result.free(resultId)
	return value
end
...
-- Function to obtain the keys of a table in the order in which they were defined
function getKeysInOrder(tbl)
    local keys = {}
    for k, _ in pairs(tbl) do
        table.insert(keys, k)
    end
    table.sort(keys)  -- Sort the keys numerically if they are numbers
    return keys
end