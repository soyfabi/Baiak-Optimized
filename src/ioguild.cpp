...
{
	Database& db = Database::getInstance();
	if (DBResult_ptr result = db.storeQuery(fmt::format("SELECT `name`, `level`, `experience` FROM `guilds` WHERE `id` = {:d}", guildId))) {
		Guild* guild = new Guild(guildId, result->getString("name"));
		guild->level = result->getNumber<uint32_t>("level");
		guild->experience = result->getNumber<uint64_t>("experience");
